...
class TestsTags(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.tag_detail_response_data = {'id': 1, 'name': 'Завтрак',
                                        'color': '#411d96', 'slug': 'breakfast'}
        cls.tag_list_response_data = [{'id': 1, 'name': 'Завтрак',
                                       'color': '#411d96', 'slug': 'breakfast'},
                                      {'id': 2, 'name': 'Oбед',
                                       'color': '#411d97', 'slug': 'lunch'}]

    def test_get_tags(self):
        response = self.client.get(self.url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(json.loads(response.content), self.tag_list_response_data)
...
class TestsIngredients(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.url_detail = reverse('api:ingredient-detail', kwargs={'pk': cls.FIRST_INGREDIENT_ID})
        cls.tag_first = Ingredient.objects.create(id=cls.FIRST_INGREDIENT_ID,
                                                  name=cls.FIRST_INGREDIENT_NAME,
                                                  measurement_unit=cls.FIRST_INGREDIENT_UNIT)
        cls.tag_second = Ingredient.objects.create(id=cls.SECOND_INGREDIENT_ID,
                                                   name=cls.SECOND_INGREDIENT_NAME,
                                                   measurement_unit=cls.SECOND_INGREDIENT_UNIT)
        cls.tag_detail_response_data = {'id': 1, 'name': 'Картошка', 'measurement_unit': 'кг'}
        cls.tag_list_response_data = [{'id': 1, 'name': 'Картошка', 'measurement_unit': 'кг'},
                                      {'id': 2, 'name': 'Сахар', 'measurement_unit': 'г'}]
...
class TestsFollowing(APITestCase):
    ...
    @classmethod
    def setUpTestData(cls):
        cls.url_first = reverse('api:foodgramuser-subscribe', kwargs={'pk': cls.FIRST_USER_ID})
        cls.url_second = reverse('api:foodgramuser-subscribe', kwargs={'pk': cls.SECOND_USER_ID})
        cls.user = User.objects.create(id=cls.FIRST_USER_ID, username=cls.FIRST_USER_USERNAME,
                                       email=cls.FIRST_USER_EMAIL, first_name=cls.FIRST_USER_FIRST_NAME,
                                       last_name=cls.FIRST_USER_LAST_NAME)
        cls.following = User.objects.create(id=cls.SECOND_USER_ID, username=cls.SECOND_USER_USERNAME,
                                            email=cls.SECOND_USER_EMAIL, first_name=cls.SECOND_USER_FIRST_NAME,
                                            last_name=cls.SECOND_USER_LAST_NAME)
        cls.follow_obj = Follow.objects.create(user_id=cls.SECOND_USER_ID, following_id=cls.FIRST_USER_ID)
        ...
        cls.post_response_data = {'email': 'user2@ya.ru', 'id': 2, 'username': 'following', 'first_name': 'user2',
                                  'last_name': 'user_second', 'is_subscribed': True, 'recipes': [], 'recipes_count': 0}

    def test_post_del_follow(self):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token_first.key)
        response = self.client.post(self.url_second)
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(json.loads(response.content), self.post_response_data)

    def test_del_follow(self):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.token_second.key)
